  ...
  const defender_buttons = defenders.map((defender, index) => {
    return (
      <div className="defender" key={index}>
        <input onChange={() => { props.onChange(defender); } } className={"form-input " + (props.defenders.indexOf(defender) >= 0 ? "checked-input" : "")} type="checkbox" id={"attacker-" + defender.toLowerCase()} checked={props.defenders.indexOf(defender) >= 0}/>
